...
def raw2outputs(
    ...
    weights = (
        ...
        * torch.cumprod(
            torch.cat(
                [torch.ones((alpha.shape[0], 1)), 1.0 - alpha + 1e-10], -1
            ),
            -1,
        )[:, :-1]
    )
...
def config_parser():
    ...
    parser.add_argument(
        "--lrate", type=float, default=5e-4, help="learning rate"
    )
    parser.add_argument(
        "--lrate_decay",
        type=int,
...
def train():
    ...
    if args.dataset_type == "llff":
        ...
        poses = poses[:, :3, :4]
        print(
            "Loaded llff", images.shape, render_poses.shape, hwf, args.datadir
        )
        ...
        images, poses, render_poses, hwf, K, i_split, near, far = (
            load_LINEMOD_data(args.datadir, args.half_res, args.testskip)
        )
        print(
